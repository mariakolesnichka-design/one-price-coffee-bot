...
def init_db():
    ...
    if cur.fetchone()[0] == 0 and SEED_PATH.exists():
        ...
        for item in data["items"]:
            cur.execute(
                "INSERT INTO items(category,name,sizes,ingredients,technology,description,serve_time) VALUES(?,?,?,?,?,?,?)",
                (item.get("category",""), item.get("name",""), item.get("sizes",""), item.get("ingredients",""), item.get("technology",""), item.get("description",""), item.get("serve_time",""))
            )
    conn.commit()
    conn.close()
...
def get_item(item_id):
    conn = db(); cur = conn.cursor()
    cur.execute("SELECT id, category, name, sizes, ingredients, technology, description, serve_time FROM items WHERE id=?", (item_id,))
...
def item_list_kb(category):
    ...
    kb.button(text="⬅️ Категории", callback_data="categories")
    kb.adjust(1)
    return kb.as_markup()
...
def item_kb(item_id):
    kb = InlineKeyboardBuilder()
    kb.button(text="⭐ В избранное", callback_data=f"fav:{item_id}")
    kb.button(text="⬅️ Категории", callback_data="categories")
    kb.button(text="🏠 Старт", callback_data="home")
    kb.adjust(1)
    return kb.as_markup()
...
async def main():
    ...
    async def admin(message: Message):
        if message.from_user.id not in ADMIN_IDS:
            ...
            return
        await message.answer("⚙️ Админ-панель\n\nПока доступно наполнение через SQLite. Позже добавим кнопки добавления/редактирования прямо в Telegram.")

    @dp.callback_query(F.data == "home")
    async def cb_home(call: CallbackQuery):
        await call.message.edit_text("☕ <b>One Price Coffee</b>\nТвой гид под рукой.\n\nВыберите раздел или напишите название напитка.", reply_markup=main_menu_kb())
        await call.answer()
    ...
    async def cb_cat(call: CallbackQuery):
        ...
        await call.message.edit_text(f"<b>{cat}</b>\nВыберите позицию:", reply_markup=item_list_kb(cat))
        await call.answer()
    ...
    async def cb_fav(call: CallbackQuery):
        item_id = int(call.data.split(":", 1)[1])
        conn = db(); cur = conn.cursor()
        cur.execute("INSERT OR IGNORE INTO favorites(user_id,item_id) VALUES(?,?)", (call.from_user.id, item_id))
    ...
    async def cb_favorites(call: CallbackQuery):
        conn = db(); cur = conn.cursor()
        cur.execute("SELECT i.id, i.name FROM items i JOIN favorites f ON i.id=f.item_id WHERE f.user_id=? ORDER BY i.name", (call.from_user.id,))
        rows = cur.fetchall(); conn.close()
        if not rows:
            await call.message.edit_text("Избранное пока пустое.", reply_markup=main_menu_kb())
        else:
            kb = InlineKeyboardBuilder()
            ...
            kb.button(text="🏠 Старт", callback_data="home")
            kb.adjust(1)
            await call.message.edit_text("⭐ Избранное:", reply_markup=kb.as_markup())
        await call.answer()

    @dp.callback_query(F.data == "search_help")
    async def cb_search_help(call: CallbackQuery):
        await call.message.edit_text("🔍 Напишите в чат название напитка или часть названия.\nНапример: <b>матча</b>, <b>сырный</b>, <b>лимонад</b>.", reply_markup=main_menu_kb())
        await call.answer()
    ...
    async def text_search(message: Message):
        ...
        if not rows:
            await message.answer("Ничего не найдено. Попробуйте другое название или откройте меню.", reply_markup=main_menu_kb())
            return
        kb = InlineKeyboardBuilder()
        for item_id, name, category in rows:
            kb.button(text=f"{name} — {category}", callback_data=f"item:{item_id}")
        kb.adjust(1)
        await message.answer("Нашла варианты:", reply_markup=kb.as_markup())
